...
<!--          <el-form-item label="推荐状态：">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button size="mini" class="btn-add" @click="handleSelectProduct()">添加</el-button>
    </el-card>
...
<script>
  import {fetchList,updateRecommendStatus,deleteGroupon,createGroupon,updateGrouponSort} from '@/api/top';
    ...
    methods: {
      ...
      handleBatchOperate(){
        ...
        for (let i = 0; i < this.multipleSelection.length; i++) {
          ids.push(this.multipleSelection[i].topId);
        }
      ...
      handleSelectDialogConfirm(){
        ...
        for (let i = 0; i < this.dialogData.multipleSelection.length; i++) {
            ids = ids + this.dialogData.multipleSelection[i].productId + ',';
          selectProducts.push({
            productId:this.dialogData.multipleSelection[i].productId
/*              ,
            productName:this.dialogData.multipleSelection[i].productName*/
          });
        }
        this.$confirm('使用要进行添加操作?', '提示', {
          ...
          createGroupon(params).then(response=>{
      ...
      handleEditSort(index,row){
        ...
        this.sortDialogData.id=row.topId;
      },
      handleUpdateSort(){
        this.$confirm('是否要修改排序?', '提示', {
          ...
          updateGrouponSort(this.sortDialogData).then(response=>{
      ...
      deleteProduct(ids){
        this.$confirm('是否要删除该推荐?', '提示', {
          ...
          let params=new URLSearchParams();
          params.append("ids",ids);
          deleteGroupon(params).then(response=>{
            this.getList();
            this.$message({
              type: 'success',
              message: '删除成功!'
