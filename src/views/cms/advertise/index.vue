  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="homeAdvertiseTable"
        ...
        <el-table-column label="广告位置" width="150" align="center">
        ...
        <el-table-column label="时间" width="260" align="center">
          <template slot-scope="scope">
            <p>开始时间：{{scope.row.startDate | formatTime}}</p>
            <p>到期时间：{{scope.row.endDate | formatTime}}</p>
          </template>
        </el-table-column>
...
<script>
  import {fetchList,updateStatus,deleteHomeAdvertise,batchDeleteHomeAdvertise} from '@/api/homeAdvertise';
  ...
  export default {
    ...
    methods: {
      ...
      handleAdd(){
        this.$router.push({path: '/cms/addAdvertise'})
      },
      handleUpdate(index,row){
        this.$router.push({path: '/cms/updateAdvertise', query: {bannerId: row.bannerId}})
      },
      getList() {
      ...
      deleteHomeAdvertise(ids){
        ...
        batchDeleteHomeAdvertise(ids){
            this.$confirm('是否要删除该广告?', '提示', {
                ...
                let params=new URLSearchParams();
                params.append("bannerId",ids);
                batchDeleteHomeAdvertise(params).then(response=>{
                    this.getList();
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
