    ...
    <div class="table-container">
      <el-table ref="deliverOrderTable"
        ...
        <el-table-column label="物流单号" width="180" align="center">
          <template slot-scope="scope">
            <el-input size="small" v-model="scope.row.deliverySn" @change="whendeliverySnChange"></el-input>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods:{
      whenSelectChange(item){
       console.log(item)
      this.list.deliveryCompany=item;
      },
      whendeliverySnChange(item){
        console.log('sss')
        console.log(item)
     
       this.list.deliverySn=item;
         console.log(this.list.deliverySn);
      },
      cancel(){
        this.$router.back();
      },
      confirm(){
        this.$confirm('是否要进行发货操作?', '提示', {
          ...
          let data={
            expressCode:this.list.deliverySn,
            expressCompany:this.list.deliveryCompany
          };
          console.log(data);
          console.log(this.list[0].orderId)
          deliveryOrder(this.list[0].orderId,data).then(response=>{
            this.$router.back();
            this.$message({
              type: 'success',
              message: '发货成功!'
