  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="returnApplyTable"
        ...
        <el-table-column label="申请时间" width="195" align="center">
          <template slot-scope="scope">{{scope.row.createdDate | formatTime}}</template>
        </el-table-column>
        ...
        <el-table-column label="处理状态" width="180" align="center">
          <template slot-scope="scope">{{scope.row.activated | formatStatus}}</template>
        </el-table-column>
        <el-table-column label="处理时间" width="195" align="center">
          <template slot-scope="scope">{{scope.row.updatedDate | formatTime}}</template>
        </el-table-column>
        <el-table-column label="处理结果" width="260" align="center">
          <template slot-scope="scope">{{scope.row.content }}</template>
        </el-table-column>
        <el-table-column label="评论" width="252" align="center">
...
<script>
  ...
  import {fetchList,deleteAppeal,batchDeleteApply} from '@/api/appeal';
  ...
  export default {
    ...
    filters:{
      ...
      formatReturnAmount(row){
          if (row.actualAmount == null || '' === row.actualAmount) {
              return '0';
          }else {
    ...
    methods:{
      ...
      handleViewDetail(index,row){
        this.$router.push({path:'/oms/returnApplyDetail',query:{id:row.appealId}})
      },
      handleBatchOperate(){
        ...
        if(this.operateType===1){
          ...
          this.$confirm('是否要进行删除操作?', '提示', {
            ...
            for(let i=0;i<this.multipleSelection.length;i++){
              ids.push(this.multipleSelection[i].opId);
            }
            params.append("ids",ids);
              batchDeleteApply(params).then(response=>{
              this.getList();
              this.$message({
                type: 'success',
                message: '删除成功!'
